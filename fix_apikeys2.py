...
text = text.replace("className={inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium }", "className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${apiKey.enabled ? 'text-success bg-success' : 'text-danger bg-danger'}`}")
text = text.replace("className={\nelative m-0 block h-6.5 w-12.5 cursor-pointer rounded-full }", "className={`relative m-0 block h-6.5 w-12.5 cursor-pointer rounded-full ${apiKey.enabled ? 'bg-primary' : 'bg-stroke dark:bg-[#313D4A]'}`}")
