...
const Spending = () => {
  const { data, loading, error, fetchData } = useApi('/finance/spending');
  ...
  return (
    <div className="rounded-xl border border-stroke dark:border-[#2E3A47] bg-white dark:bg-[#24303F] p-6 shadow-default h-full flex flex-col justify-between">
      <DataState 
        ...
        error={error} 
        onRetry={fetchData} 
        isEmpty={!data || !data.categories || data.categories.length === 0} 
        skeleton={
          <div className="h-full w-full animate-pulse flex flex-col pt-2">
            ...
            <div className="w-full h-4 rounded-full bg-gray-200 dark:bg-gray-700 mb-8"></div>
            <div className="grid grid-cols-2 gap-y-4 gap-x-2 mt-auto">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-gray-200 dark:bg-gray-700 shrink-0"></div><div className="h-3 w-16 bg-gray-200 dark:bg-gray-700 rounded"></div></div>
              ))}
            </div>
          </div>
        }
