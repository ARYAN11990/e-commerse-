...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // For demo purposes, if they enter an email instead of the username
      // we still pass it to login. If it fails, they will see an error.
      // In this specific template backend, default username is "ARYAN PARMAR"
  ...
  const handleSocialLogin = (provider) => {
    ...
    setTimeout(async () => {
      try {
        // Authenticate as default mock user
  ...
  return (
    <AuthLayout>
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-[#1C2434] dark:text-white mb-2">Sign In</h2>
        <p className="text-[#64748B] dark:text-[#8A99AF] font-medium">Enter your email and password to sign in!</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 mb-8">
        ...
        >
          ...
          <span className="font-medium text-[#1C2434] dark:text-white">Sign in with Google</span>
        </button>
        <button 
        ...
        >
          ...
          <span className="font-medium text-[#1C2434] dark:text-white">Sign in with X</span>
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-6 flex items-center justify-between gap-2">
          <label className="flex cursor-pointer items-center gap-3">
            ...
            <span className="font-medium text-[#64748B] dark:text-[#8A99AF]">Keep me logged in</span>
          </label>
          <Link to="/forgot-password" className="font-medium text-[#3C50E0] hover:underline">
            Forgot password?
          </Link>
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="font-medium text-[#64748B] dark:text-[#8A99AF]">
            ...
            <Link to="/register" className="text-[#3C50E0] hover:underline">
              Sign Up
            </Link>
          </p>
        </div>
